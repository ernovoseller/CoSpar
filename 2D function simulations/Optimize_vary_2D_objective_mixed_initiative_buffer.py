...
This version: preference buffer (n = 1, b > 0); with coactive feedback.
"""
...
save_folder = 'Sim_results/Buffer_dueling_mixed_initiative_take2/'
...
for i, run_num in enumerate(run_nums):
    ...
    for trial in range(num_trials):
        ...
        if np.size(coactive_feedback) > 0:
            ...
            virtual_sample_idxs = np.concatenate((virtual_sample_idxs, 
                                                  [pref_count - 1]))

    # Save the results for this experimental run:
    io.savemat(save_folder + 'Opt_2D_900_buffer_' + str(buffer_size) + \
               '_vary_obj_run_' + str(run_num) + '.mat', {'data_pts': data_pts, 
